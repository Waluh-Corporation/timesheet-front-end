...
import type { Site, SiteRequest } from "@/lib/types";
import { fetchSites, createSite, updateSite, deleteSite } from "@/app/(admin)/master-data/services/masterData";
...
export function SitesTable() {
  ...
  const [form, setForm] = useState<SiteRequest & { id?: number }>({
    code: "",
    name: "",
    is_active: true,
  });
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      notify(err.message || "Failed to load sites", "error");
    } finally {
      setLoading(false);
    }
  }, [notify]);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (form.id) {
        ...
        notify("Site created", "success");
      }
      setModalOpen(false);
      loadData();
    } catch (err: any) {
      notify(err.message || "Failed to save site", "error");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="card p-6">
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-lg font-bold">Sites ({totalItems})</h2>
      ...
      {loading ? (
        ...
        <>
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm" data-testid="sites-table">
              <thead>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
          <div className="card w-full max-w-md p-6 shadow-hard">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-lg font-bold">{form.id ? "Edit Site" : "New Site"}</h2>
            ...
            <form onSubmit={handleSave} className="flex flex-col gap-3">
              ...
              <div className="mt-2 flex justify-end gap-2">
                ...
                <button type="submit" className="btn-primary" disabled={saving}>
                  {saving && <Loader2 size={16} className="animate-spin" />} Save Site
                </button>
              </div>
            </form>
          </div>
