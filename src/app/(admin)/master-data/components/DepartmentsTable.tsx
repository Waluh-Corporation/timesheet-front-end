...
export function DepartmentsTable() {
  ...
  const [form, setForm] = useState<DepartmentRequest & { id?: number }>({
    code: "",
    name: "",
    division: "",
    is_active: true,
  ...
  const {
    ...
    totalItems,
    totalPages,
  } = useClientPagination<Department>(
    departments,
  ...
  return (
    <div className="card p-6">
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-lg font-bold">Departments ({totalItems})</h2>
        <div className="flex items-center gap-2">
          ...
          <button
            data-testid="create-departments-btn"
            onClick={() => {
              setForm({ code: "", name: "", division: "", is_active: true });
              setModalOpen(true);
            }}
      ...
      {loading ? (
        ...
        <>
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm" data-testid="departments-table">
              <thead>
                <tr className="border-b-2 border-mr-ink text-xs uppercase text-mr-muted">
                  ...
                  <th className="pb-2 cursor-pointer hover:text-mr-ink" onClick={() => handleSort("division")}>
                    <div className="flex items-center gap-1">
                      Division {sortKey === "division" && (sortDir === "asc" ? <ChevronUp size={14} /> : <ChevronDown size={14} />)}
                    </div>
                  </th>
              ...
              <tbody>
                {paginatedData.map((d) => (
                  <tr key={d.id} className="border-b border-mr-ink/20 hover:bg-mr-surface2/50">
                    ...
                    <td className="py-3 text-mr-muted">{d.division || "-"}</td>
                    <td className="py-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-1">
                  <button
                    ...
                    disabled={page === 1}
                    className="p-1 hover:bg-mr-surface2 disabled:opacity-50"
                  >
                    <ChevronLeft size={16} />
                  </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
          <div className="card w-full max-w-md p-6 shadow-hard">
            <div className="mb-4 flex items-center justify-between">
              ...
              <button onClick={() => setModalOpen(false)} className="p-1 hover:bg-mr-surface2">
                <X size={18} />
              </button>
            </div>
